...
# def unique_callers(seq):
#     seen = set()
#     out = []
#     for item in seq:
#         key = (item.get("file"), item.get("function"))
#         if key not in seen:
#             seen.add(key)
#             out.append(item)
#     return out
...
#                 notify_about_function_behaviour.extend(callers)
#     # remove duplicates by converting each dict to a tuple key
    
#         notify_about_function_behaviour = unique_callers(notify_about_function_behaviour)
#     seen = set()
#     unique_list = []
